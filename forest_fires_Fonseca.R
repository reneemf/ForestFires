...
# Import & view forest fires dataset as data frame
forest_fires <- read_csv('forestfires.csv')
summary(forest_fires)
head(forest_fires)

#     data is taken from a 2006 forest fire prevention pub
...
# Mutate df to put months & days in chronological order 
...
# Fires seem to occur more in the late summer/early fall, especially
# August, and September followed by March. They allow occur more on 
# the weekend (Sunday, Friday, and Saturday in order)

# How are variables linked to forest fires, like temperature, rainfall, etc.,
# distributed among the months and days measured?
...
# temp & DC (drought code, measures dry conditions) are both high in the summer 
# & early fall months when more forest fires are occurring.

# Are these late summer/early fall forest fires more or less severe than
# the fires throughout the rest of the year?
...
# Scatterplots do not show a very clear pattern, possibly due to outliers
# which will need to be addressed. Subsetting data may help in understanding how different variables contribute
# to fire spread (area) w/o the interference of outliers
...
# Plots have lots of data points clustered at the bottom. How would removing
# outliers from the bottom of the IQR affect them?
...
# forest_fires has entries of 0 as the area of some fires which seems suspect
# and is limiting how predictive my scatters can be
summary(forest_fires)
head(forest_fires)
...
# Future plans: 
#       compress all these plotting functions into one that takes
#       any data set, any x & any y 
#       and put plots in the same screen(such as using the code: par(mfrow=c(n,n))), 
